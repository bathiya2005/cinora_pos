...
export function BillTemplateSettings() {
  ...
  // Per-branch bill identity (name + logo shown on that branch's own
  // Navbar and printed bills) — editable centrally from here by the admin.
  ...
  const fetchBranches = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const branchUsers = data.filter((u) => u.role === 'branch');
        setBranches(branchUsers);
        if (branchUsers.length > 0 && !selectedBranchId) {
          setSelectedBranchId(branchUsers[0].id);
        }
      }
    } catch {
      // ignore
  ...
  const handleBranchLogoFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          setBranchLogoUrl(reader.result);
        }
      };
  ...
  const handleSaveBranchIdentity = async () => {
    if (!selectedBranchId) return;
    if (!branchCompanyName.trim()) {
      addToast('Enter a bill name for this branch', 'error');
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto animate-fadeIn">
      ...
      {/* Per-Branch Bill Identity (name + logo only — rest of template stays shared) */}
      <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-2xl p-6 shadow-md space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
          <h3 className="font-bold text-slate-900 dark:text-white text-base flex items-center gap-2">
            <Store className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> Branch Bill Identity
          </h3>
        ...
        <p className="text-xs text-slate-600 dark:text-slate-500 -mt-2">
          Set the name and logo that appear on each branch's own Navbar and printed bills. Everything else (phone, address, footer) stays shared from the template below.
        </p>

        {branches.length === 0 ? (
          <p className="text-xs text-slate-500 py-2">No branch accounts yet — create one from Branch Accounts.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Select Branch</label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Bill / Navbar Name</label>
              <input
                type="text"
                value={branchCompanyName}
                onChange={(e) => setBranchCompanyName(e.target.value)}
                placeholder="e.g. Cinora Spice Exports (PVT) Ltd"
                className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                {branchLogoUrl && (
                  <img src={branchLogoUrl} alt="Branch logo" className="w-9 h-9 rounded-lg object-contain bg-white border border-slate-300 shrink-0" />
                )}
                <label className="flex-1 px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-xl cursor-pointer border border-slate-300 dark:border-slate-700 inline-flex items-center justify-center gap-1.5 transition-colors">
                  <ImageIcon className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                  <span>Upload Logo</span>
                  <input type="file" accept="image/*" onChange={handleBranchLogoFileUpload} className="hidden" />
                </label>
                {branchLogoUrl && (
                  <button type="button" onClick={() => setBranchLogoUrl('')} className="text-xs text-rose-600 dark:text-rose-400 hover:underline shrink-0">
                    Remove
                  </button>
                )}
              </div>
            ...
            <div className="sm:col-span-2">
              <button
                type="button"
                onClick={handleSaveBranchIdentity}
                disabled={savingBranch}
                className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-bold rounded-xl text-xs shadow-md flex items-center gap-2 transition-colors"
              >
                <Save className="w-4 h-4" /> {savingBranch ? 'Saving...' : 'Save Branch Identity'}
              </button>
            </div>
          </div>
        )}
